...
from backend.app.services.team_billing_service import (
    capture_reservation_and_charge_team,
    release_reservation_team
)
...
def process_bulk_task(self, job_id: str):
    ...
    try:
        ...
        json_obj = f"outputs/{job.job_id}.json"
        csv_obj  = f"outputs/{job.job_id}.csv"
        ...
        writer.writerow(["email", "status", "risk_score"])

        for r in results:
            if "result" in r:
                rr = r["result"]
                writer.writerow([r["email"], rr.get("status"), rr.get("risk_score")])
            else:
                writer.writerow([r["email"], "error", ""])
        ...
        for res in reservations:
            try:
                ...
                if res.reference and str(res.reference).startswith("team:"):
                    _, tid, *_ = res.reference.split(":")
                    tid = int(tid)

                    if remain > 0:
                        capture_reservation_and_charge_team(res.id, type_="bulk_charge", reference=f"bulk:{job.job_id}")
                        remain -= Decimal(res.amount)
                    else:
                        release_reservation_team(res.id)

                # User reservation
                else:
                    if remain > 0:
                        capture_reservation(db, res.id, type_="bulk_charge", reference=f"bulk:{job.job_id}")
                        remain -= Decimal(res.amount)
                    else:
                        res.locked = False
                        db.add(res)
                ...
                logger.exception("Reservation finalize failed for reservation %s", res.id)
                res.locked = False
                db.add(res)
